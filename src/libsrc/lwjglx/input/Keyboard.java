...
public class Keyboard extends GLFWKeyCallback {
	...
	public static final int KEYBOARD_SIZE = 1024;
	private static EventQueue queue = new EventQueue(KEYBOARD_SIZE);
	...
	private static String[] keyName = new String[KEYBOARD_SIZE];
	private static Map<String, Integer> keyMap = new HashMap<String, Integer>(253);
	public static boolean enableRepeat = false;
	private static boolean[] keyEventIsRepeat;
	...
	public static void addKeyEvent(int var0, boolean var1, boolean var2) {
		keyEvents[queue.getNextPos()] = KeyCodes.toLwjglKey(var0);
		keyEventStates[queue.getNextPos()] = var1;
		nanoTimeEvents[queue.getNextPos()] = Sys.getNanoTime();
		keyEventChars[queue.getNextPos()] = 0;
		keyEventIsRepeat[queue.getNextPos()] = var2;
		queue.add();
	}
	
	public static void addCharEvent(int key, char c) {
		keyEvents[queue.getNextPos()] = 0;
		keyEventStates[queue.getNextPos()] = true;
		nanoTimeEvents[queue.getNextPos()] = Sys.getNanoTime();
		keyEventChars[queue.getNextPos()] = c;
		keyEventIsRepeat[queue.getNextPos()] = false;
		queue.add();
	}
	...
	public static char getEventCharacter() {
		return keyEventChars[queue.getCurrentPos()];
	}
	...
	static {
		keyEvents = new int[queue.getMaxEvents()];
		keyEventStates = new boolean[queue.getMaxEvents()];
		nanoTimeEvents = new long[queue.getMaxEvents()];
		keyEventChars = new char[queue.getMaxEvents()];
		keyName = new String[queue.getMaxEvents()];
		keyMap = new HashMap(queue.getMaxEvents());
		keyEventIsRepeat = new boolean[queue.getMaxEvents()];
	...
	public void invoke(long window, int key, int scancode, int action, int mods) {
		if (key == GLFW.GLFW_KEY_W && (mods & GLFW.GLFW_MOD_SHIFT) != 0) {
			return;
		}
	}
}
