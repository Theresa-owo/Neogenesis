...
public class ContextCapabilities {
	...
	public ContextCapabilities() {
		// Copy from LWJGL3's GLCapabilities (same field naming). Reading from
		// GLContext.getCapabilities() here would see its not-yet-assigned static
		// field (null) and silently leave every capability at its default.
		GLCapabilities caps = GL.getCapabilities();
		for (Field field : ContextCapabilities.class.getFields()) {
			String name = field.getName();
			if (name.startsWith("GL_") || name.startsWith("OpenGL")) {
				try {
					field.setBoolean(this, GLCapabilities.class.getField(name).getBoolean(caps));
				} catch (NoSuchFieldException ignored) {
					// capability does not exist in this LWJGL3 build - keep default
