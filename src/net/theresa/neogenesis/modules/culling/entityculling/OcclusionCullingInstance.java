...
public class OcclusionCullingInstance {
    ...
    public boolean isAABBVisible(Vec3d aabbMin, Vec3d aabbMax, Vec3d viewerPosition) {
        try {
            ...
            if (relX == Relative.INSIDE && relY == Relative.INSIDE &&
                relZ == Relative.INSIDE) {
                return true;
            } else {
                this.skipList.clear();
                int id = 0;
                ...
                for (int x = minX; x <= maxX; ++x) {
                    for (int y = minY; y <= maxY; ++y) {
                        for (int z = minZ; z <= maxZ; ++z) {
                            int k = this.getCacheValue(x, y, z);
                            if (k == 1) {
                                return true;
                            }
                ...
                for (int x = minX; x <= maxX; ++x) {
                    byte faceEdgeDataX = (byte) ((x == minX ? 1 : 0));
                    faceEdgeDataX = (byte) (faceEdgeDataX | (x == maxX ? 2 : 0));
                    byte visibleOnFaceX = (byte) ((x == minX && relX == Relative.POSITIVE ? 1 : 0));
                    visibleOnFaceX = (byte) (visibleOnFaceX |
                                             (x == maxX && relX == Relative.NEGATIVE ? 2 : 0));

                    for (int y = minY; y <= maxY; ++y) {
                        byte faceEdgeDataY = (byte) (faceEdgeDataX | (y == minY ? 4 : 0));
                        faceEdgeDataY = (byte) (faceEdgeDataY | (y == maxY ? 8 : 0));
                        byte visibleOnFaceY = (byte) (visibleOnFaceX |
                                                      (y == minY && relY == Relative.POSITIVE
                                                       ? 4
                                                       : 0));
                        visibleOnFaceY = (byte) (visibleOnFaceY |
                                                 (y == maxY && relY == Relative.NEGATIVE
                                                  ? 8
                                                  : 0));
                        ...
                        for (int z = minZ; z <= maxZ; ++z) {
                            ...
                            byte visibleOnFace = (byte) (visibleOnFaceY | (z == minZ && relZ ==
                                                                                        Relative.POSITIVE
                                                                           ? 16
                                                                           : 0));
                            visibleOnFace = (byte) (visibleOnFace |
                                                    (z == maxZ && relZ == Relative.NEGATIVE
                                                     ? 32
                                                     : 0));
                            if (this.skipList.get(id)) {
                                ...
                                if (visibleOnFace != 0) {
                                    ...
                                    if (this.isVoxelVisible(viewerPosition, this.targetPos, faceEdgeData,
                                                            visibleOnFace)) {
                                        return true;
                                    }
                                }
    ...
    private boolean isVisible(Vec3d start, Vec3d[] targets, int size) {
        ...
        for (int v = 0; v < size; ++v) {
            ...
            if (!this.allowRayChecks || !this.rayIntersection(this.lastHitBlock, start, (new Vec3d(relativeX, relativeY,
                                                                                                   relativeZ)).normalize())) {
                ...
                boolean finished =
                        this.stepRay(start, x, y, z, dimFracX, dimFracY, dimFracZ, intersectCount, x_inc, y_inc, z_inc,
