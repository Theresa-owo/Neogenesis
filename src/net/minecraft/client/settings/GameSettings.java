...
import net.theresa.neogenesis.modules.render.FullBright;
...
public class GameSettings {
    ...
    private static final String[] PARTICLES = { "options.particles.all", "options.particles.decreased",
    ...
    public KeyBinding keyBindSprint = AutoSprint.Instance;
    ...
    public KeyBinding[] keyBindsHotbar = { new KeyBinding("key.hotbar.1", 2, "key.categories.inventory"),
    ...
    public int thirdPersonView() {
        if (this.mc.thePlayer != null && this.mc.thePlayer.isEntityInsideOpaqueBlock()) {
            return 0;
        } else if (FreeLookUtil.Instance.getPerspectiveToggled()) {
            return 1;
        }
    ...
    public float gammaSetting() {
        if (FullBright.Instance.getToggled()) {
            return 1024.0f;
        } else {
            return gammaSetting;
        }
    ...
    private static final String[] KEYS_DYNAMIC_LIGHTS = { "options.off", "options.graphics.fast",
    ...
    public GameSettings(Minecraft mcIn, File optionsFileIn) {
        ...
        this.keyBindings = ArrayUtils.addAll(this.keyBindings, KeySettings.getKeyBindings());
    ...
    public static String getKeyDisplayString(int key) {
        return key < 0 ? I18n.format("key.mouseButton", key + 101)
                                            ...
                                            : settingOption == GameSettings.Options.RENDER_CLOUDS
                                                    ...
                                                    : settingOption == GameSettings.Options.GAMMA
                                                            ? f == 0.0F ? s1 + I18n.format("options.gamma.min")
                                                                    : f == 1.0F ? s1 + I18n.format("options.gamma.max")
                                                                            : s1 + "+" + (int) (f * 100.0F) + "%"
                    ...
                    while ((s = bufferedreader.readLine()) != null) {
                        try {
                            ...
                            if (astring[0].equals("resourcePacks")) {
                                this.resourcePacks = (List) gson.fromJson(s.substring(s.indexOf(58) + 1),
                                        typeListString);
                            ...
                            if (astring[0].equals("incompatibleResourcePacks")) {
                                this.incompatibleResourcePacks = (List) gson.fromJson(s.substring(s.indexOf(58) + 1),
                                        typeListString);
    ...
    private float getOptionFloatValueOF(GameSettings.Options p_getOptionFloatValueOF_1_) {
        ...
        if (p_getOptionFloatValueOF_1_ == GameSettings.Options.AO_LEVEL) {
            ...
            return this.limitFramerate == GameSettings.Options.FRAMERATE_LIMIT.getValueMax() && this.enableVsync ? 0.0F
                    : (float) this.limitFramerate;
    ...
    private String getKeyBindingOF(GameSettings.Options p_getKeyBindingOF_1_) {
        ...
        if (p_getKeyBindingOF_1_ == GameSettings.Options.FOG_FANCY) {
            switch (this.ofFogType) {
            case 1:
