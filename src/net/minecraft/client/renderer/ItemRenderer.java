...
public class ItemRenderer {
    ...
    public static boolean isItem3D = false;
    public static boolean isItemSword = false;
    public static boolean isInvertLighting = false;
    ...
    private void setLightMapFromPlayer(AbstractClientPlayer clientPlayer) {
        int i = this.mc.theWorld.getCombinedLight(
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress) {
        float diff1 = 0.0f, diff2 = 0.0f, diff3 = 0.0f, diff4 = 0.0f;
        float diff7 = 0.0f;
        float f = MathHelper.sin(swingProgress * swingProgress * (float) Math.PI);
        float f1 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI);
        GlStateManager.translate(0.56F + diff1, -0.52F + diff3, -0.72F + diff4);
        GlStateManager.translate(0.0F, equipProgress * -0.6F, 0.0F);
        GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(f * -20.0F + diff2, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(f1 * -20.0F, 0.0F, 0.0F, 1.0F);
        GlStateManager.rotate(f1 * (diff7 - 80.0F), 1.0F, 0.0F, 0.0F);
    ...
    public void transformFirstPersonItemOld(float f, float f1) {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() /*|| !Shaders.isSkipRenderHand()*/) {
            if (!Config.isShaders() /*|| !Shaders.isSkipRenderHand()*/) {
                boolean noequip = NoSwingingItem.Instance.getToggled() && ItemAnimations.Instance.getToggled();
                float f = noequip
                        ? 0.0f
                        : 1.0F - (this.prevEquippedProgress +
                        (this.equippedProgress - this.prevEquippedProgress) * partialTicks);
                AbstractClientPlayer abstractclientplayer = this.mc.thePlayer;
                float f1 = SwingUtil.INSTANCE.getSwingProgress(partialTicks);
                ...
                GlStateManager.pushMatrix();
                ItemStack thisitemToRender = noequip ? Registry.Companion.getMc().thePlayer.getCurrentEquippedItem() : this.itemToRender;
                if (thisitemToRender != null) {
                    if (thisitemToRender.getItem() instanceof ItemMap) {
                        this.renderItemMap(thisitemToRender, abstractclientplayer, f2, f, f1);
                    } else if (abstractclientplayer.getItemInUseCount() > 0) {
                        EnumAction enumaction = thisitemToRender.getItemUseAction();

                        switch (enumaction) {
                            case NONE:
                                this.transformFirstPersonItemOld(f, f1);
                                break;
                            ...
                            case BLOCK:
                                if (NoBlockhitting.Instance.getToggled() && ItemAnimations.Instance.getToggled()) {
                    ...
                    isRenderingArm = true;
                    isItem3D = this.itemRenderer.shouldRenderItemIn3D(thisitemToRender);
                    isItemSword = thisitemToRender.getItemUseAction() == EnumAction.BLOCK;
                    isInvertLighting = !isItem3D && InvLight.Instance.getToggled() && ItemAnimations.Instance.getToggled();
                    double scale = ItemAnimations.Instance.getToggled() ? ItemScale.Instance.getScrollValue() : 1.0;
                    GlStateManager.pushMatrix();
                    double x = ItemAnimations.Instance.getToggled() ? ItemPositionX.Instance.getScrollValue() : 0.0;
                    double y = ItemAnimations.Instance.getToggled() ? ItemPositionY.Instance.getScrollValue() : 0.0;
                    double z = ItemAnimations.Instance.getToggled() ? ItemPositionZ.Instance.getScrollValue() : 0.0;
                    GlStateManager.translate(x, y, z);
                    GlStateManager.scale(scale, scale, scale);
                    this.renderItem(abstractclientplayer, thisitemToRender,
                            ItemCameraTransforms.TransformType.FIRST_PERSON);
                    GlStateManager.popMatrix();
                    isInvertLighting = false;
        ...
        private void renderItemMap(ItemStack thisitemToRender, AbstractClientPlayer clientPlayer, float pitch,
        float equipmentProgress, float swingProgress) {
            ...
            worldrenderer.pos(-7.0D, -7.0D, 0.0D).tex(0.0D, 0.0D).endVertex();
            tessellator.draw();
            MapData mapdata = Items.filled_map.getMapData(thisitemToRender, this.mc.theWorld);
