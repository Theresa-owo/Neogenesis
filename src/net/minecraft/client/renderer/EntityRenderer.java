...
import net.theresa.neogenesis.modules.render.BetterSky;
...
import libsrc.lwjglx.util.glu.Project;
...
import net.theresa.neogenesis.utils.moduleutils.ZoomUtil;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public float persPrevYaw, persYaw;
    public float persPrevPitch, persPitch;
    ...
    private static final ResourceLocation[] shaderResourceLocations = { new ResourceLocation("shaders/post/notch.json"),
    ...
    public static double lastModifier = 0.0;
        ...
        if (this.debugView) {
            ...
            if (flag) {
                if (!Config.zoomMode) {
                    ...
                    this.mc.gameSettings.smoothCamera = SmoothZoom.Instance.getToggled();
            ...
            double zoomModifier = 1.0;

            if (Config.zoomMode) {
                zoomModifier = 1.0 / Magnification.Instance.getScrollValue();
            }
            zoomModifier = ZoomUtil.INSTANCE.getZoomModifier(zoomModifier, Config.zoomMode);

            if (zoomModifier != lastModifier) {
                lastModifier = zoomModifier;
                this.mc.renderGlobal.displayListEntitiesDirty = true;
            }
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (NoHurtEffect.Instance.getToggled()) return;
    ...
    private void orientCamera(float partialTicks) {
        ...
        float modifiedYaw = FreeLookUtil.Instance.getPerspectiveToggled() ? FreeLookUtil.Instance.getCameraYaw() : entity.rotationYaw;
        float modifiedPrevYaw = FreeLookUtil.Instance.getPerspectiveToggled() ? FreeLookUtil.Instance.getCameraYaw() : entity.prevRotationYaw;
        float modifiedPitch = FreeLookUtil.Instance.getPerspectiveToggled() ? FreeLookUtil.Instance.getCameraPitch() : entity.rotationPitch;
        float modifiedPrevPitch = FreeLookUtil.Instance.getPerspectiveToggled() ? FreeLookUtil.Instance.getCameraPitch() : entity.prevRotationPitch;
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                GlStateManager.rotate(
                        modifiedPrevYaw + (modifiedYaw - modifiedPrevYaw) * partialTicks + 180.0F,
                        0.0F, -1.0F, 0.0F);
                GlStateManager.rotate(
                        modifiedPrevPitch + (modifiedPitch - modifiedPrevPitch) * partialTicks,
                        -1.0F, 0.0F, 0.0F);
            }
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                float f1 = modifiedYaw;
                float f2 = modifiedPitch;

                if (this.mc.gameSettings.thirdPersonView() == 2) {
                    f2 += 180.0F;
                }

                double d4 = -MathHelper.sin(f1 / 180.0F * (float) Math.PI)
                        * MathHelper.cos(f2 / 180.0F * (float) Math.PI) * d3;
                double d5 = MathHelper.cos(f1 / 180.0F * (float) Math.PI)
                        * MathHelper.cos(f2 / 180.0F * (float) Math.PI) * d3;
                ...
                GlStateManager.rotate(modifiedPitch - f2, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(modifiedYaw - f1, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(0.0F, 0.0F, (float) -d3);
                GlStateManager.rotate(f1 - modifiedYaw, 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f2 - modifiedPitch, 1.0F, 0.0F, 0.0F);
            }
        } else {
        ...
        if (!this.mc.gameSettings.debugCamEnable) {
            GlStateManager.rotate(
                    modifiedPrevPitch + (modifiedPitch - modifiedPrevPitch) * partialTicks, 1.0F,
                    0.0F, 0.0F);
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    crashreportcategory.addCrashSectionCallable("Mouse location",
                            () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d)", Integer.valueOf(k1),
    ...
    public Frustum cameraCache = null;
        ...
        if (f != 0.0F && Config.isRainSplash()) {
            ...
            for (int l = 0; l < k; ++l) {
                ...
                if (blockpos1.getY() <= blockpos.getY() + i && blockpos1.getY() >= blockpos.getY() - i
                    ...
                    if (block.getMaterial() == Material.lava) {
                        this.mc.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, blockpos1.getX() + d3,
                                blockpos1.getY() + 0.1F - block.getBlockBoundsMinY(), blockpos1.getZ() + d4, 0.0D, 0.0D,
                                0.0D);
                        ...
                        this.mc.theWorld.spawnParticle(EnumParticleTypes.WATER_DROP, blockpos2.getX() + d3,
                                blockpos2.getY() + 0.1F + block.getBlockBoundsMaxY(), blockpos2.getZ() + d4, 0.0D, 0.0D,
                                0.0D);
                    }
                }
            }
            ...
            if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
                ...
                if (d1 > blockpos.getY() + 1
                        && world.getPrecipitationHeight(blockpos).getY() > MathHelper.floor_float(blockpos.getY())) {
                    ...
                    if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
                        ...
                        if (k2 != l2) {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F) {
                                ...
                                int k3 = j3 >> 16 & 65535;
                                int l3 = j3 & 65535;
                                worldrenderer.pos(l1 - d3 + 0.5D, k2, k1 - d4 + 0.5D).tex(0.0D, k2 * 0.25D + d5)
                                        .color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                                worldrenderer.pos(l1 + d3 + 0.5D, k2, k1 + d4 + 0.5D).tex(1.0D, k2 * 0.25D + d5)
                                        .color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                                worldrenderer.pos(l1 + d3 + 0.5D, l2, k1 + d4 + 0.5D).tex(1.0D, l2 * 0.25D + d5)
                                        .color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                                worldrenderer.pos(l1 - d3 + 0.5D, l2, k1 - d4 + 0.5D).tex(0.0D, l2 * 0.25D + d5)
                                        .color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                            } else {
                                if (j1 != 1) {
                                    if (j1 >= 0) {
                                ...
                                int j4 = i4 >> 16 & 65535;
                                int k4 = i4 & 65535;
                                worldrenderer.pos(l1 - d3 + 0.5D, k2, k1 - d4 + 0.5D)
                                        .tex(0.0D + d9, k2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4)
                                        .lightmap(j4, k4).endVertex();
                                worldrenderer.pos(l1 + d3 + 0.5D, k2, k1 + d4 + 0.5D)
                                        .tex(1.0D + d9, k2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4)
                                        .lightmap(j4, k4).endVertex();
                                worldrenderer.pos(l1 + d3 + 0.5D, l2, k1 + d4 + 0.5D)
                                        .tex(1.0D + d9, l2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4)
                                        .lightmap(j4, k4).endVertex();
                                worldrenderer.pos(l1 - d3 + 0.5D, l2, k1 - d4 + 0.5D)
                                        .tex(0.0D + d9, l2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4)
                                        .lightmap(j4, k4).endVertex();
                            }
                        }
                    }
    ...
    private void updateFogColor(float partialTicks) {
        ...
        if (d1 < 1.0D && !BetterSky.Instance.getToggled()) {
            if (d1 < 0.0D) {
                d1 = 0.0D;
            }
    ...
    public boolean setFxaaShader(int p_setFxaaShader_1_) {
        ...
        if (this.theShaderGroup != null && this.theShaderGroup != this.fxaaShaders[2]
