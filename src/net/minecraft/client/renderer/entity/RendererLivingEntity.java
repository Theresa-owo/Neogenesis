...
import net.theresa.neogenesis.utils.moduleutils.*;
import net.theresa.neogenesis.utils.*;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    private IChatComponent displayName(T entity) {
        return entity.getDisplayName();
    }

    public void renderName(T entity, double x, double y, double z) {
        if (this.canRenderName(entity) && (!entity.loadedExtra || !entity.extraNoRenderName)) {
            double d0 = entity.getDistanceSqToEntity(this.renderManager.livingPlayer);
            float f = entity.isSneaking() ? NAME_TAG_RANGE_SNEAK : NAME_TAG_RANGE;

            if (d0 < (double) (f * f)) {
                String s = displayName(entity).getFormattedText();
                GlStateManager.alphaFunc(516, 0.1F);

                if (entity.isSneaking()) {
                    GlStateManager.pushMatrix();
                    GlStateManager.translate((float) x, (float) y + entity.height + 0.5F -
                            (entity.isChild() ? entity.height / 2.0F : 0.0F), (float) z);
                    ...
                    NameTagUtil.INSTANCE.prepare(entity);
                    GlStateManager.disableTexture2D();
                    NameTagUtil.INSTANCE.renderBackgound(0);
                    GlStateManager.enableTexture2D();
                    GlStateManager.depthMask(true);
                    NameTagUtil.INSTANCE.render(553648127, 0);
                    GlStateManager.enableLighting();
                    GlStateManager.disableBlend();
                    GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                    GlStateManager.popMatrix();
                    ...
                    this.renderOffsetLivingLabel(entity, x,
                            y - (entity.isChild() ? (double) (entity.height / 2.0F) : 0.0D), z, s,
                            0.02666667F, d0);
                }
            }
        }
    ...
    public boolean canRenderName(T entity) {
        ...
        if (SelfNameTag.Instance.getToggled() && entity == entityplayersp) { return true; }
