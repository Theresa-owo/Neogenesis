...
public abstract class Render<T extends Entity> implements IEntityRenderer {
    ...
    public void rotatePers() {
        if (FreeLook.Instance.getToggled()) {
            float yaw = Registry.Companion.getMc().entityRenderer.persPrevYaw +
                    (Registry.Companion.getMc().entityRenderer.persYaw - Registry.Companion.getMc().entityRenderer.persPrevYaw) *
                            Registry.Companion.getMc().timer.renderPartialTicks;
            float pitch = Registry.Companion.getMc().entityRenderer.persPrevPitch +
                    (Registry.Companion.getMc().entityRenderer.persPitch - Registry.Companion.getMc().entityRenderer.persPrevPitch) *
                            Registry.Companion.getMc().timer.renderPartialTicks;
            GlStateManager.rotate(-yaw, 0.0F, 1.0F, 0.0F);
            GlStateManager.rotate(pitch, 1.0F, 0.0F, 0.0F);
        } else {
            GlStateManager.rotate(-this.renderManager.playerViewY, 0.0F, 1.0F, 0.0F);
            if (Registry.Companion.getMc().gameSettings.thirdPersonView() == 2) {
    ...
    public static void undoRotatePers() {
        if (FreeLook.Instance.getToggled()) {
            float var0 = Registry.Companion.getMc().entityRenderer.persPrevYaw + (Registry.Companion.getMc().entityRenderer.persYaw - Registry.Companion.getMc().entityRenderer.persPrevYaw) * Registry.Companion.getMc().timer.renderPartialTicks;
            float var1 = Registry.Companion.getMc().entityRenderer.persPrevPitch + (Registry.Companion.getMc().entityRenderer.persPitch - Registry.Companion.getMc().entityRenderer.persPrevPitch) * Registry.Companion.getMc().timer.renderPartialTicks;
            GlStateManager.rotate(-var1, 1.0F, 0.0F, 0.0F);
            GlStateManager.rotate(var0, 0.0F, 1.0F, 0.0F);
        } else {
            if (Registry.Companion.getMc().gameSettings.thirdPersonView() == 2) {
                GlStateManager.rotate(Registry.Companion.getMc().getRenderManager().playerViewX, 1.0F, 0.0F, 0.0F);
            } else {
                GlStateManager.rotate(-Registry.Companion.getMc().getRenderManager().playerViewX, 1.0F, 0.0F, 0.0F);
            }

            GlStateManager.rotate(Registry.Companion.getMc().getRenderManager().playerViewY, 0.0F, 1.0F, 0.0F);
        }

    }

    public void rotatePersNeg() {
        if (FreeLook.Instance.getToggled()) {
            float yaw = Registry.Companion.getMc().entityRenderer.persPrevYaw +
                    (Registry.Companion.getMc().entityRenderer.persYaw - Registry.Companion.getMc().entityRenderer.persPrevYaw) *
                            Registry.Companion.getMc().timer.renderPartialTicks;
    ...
    public void renderName(Entity entity, double x, double y, double z) {
        if (this.canRenderName((T) entity)) {
            this.renderLivingLabel((T) entity, entity.getDisplayName().getFormattedText(), x, y, z, 64, false);
        }
    }
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance,
        ...
        if (d0 <= (double) (maxDistance * maxDistance)) {
            ...
            if (hideEntity) {
                NameTagUtil.prepareName(str);
            } else {
                NameTagUtil.prepare(entityIn);
            }*/
            GlStateManager.disableTexture2D();
            //NameTagUtil.renderBackgound(i);
            GlStateManager.enableTexture2D();
            //NameTagUtil.render(553648127, i);
            GlStateManager.enableDepth();
            GlStateManager.depthMask(true);
            //NameTagUtil.render(-1, i);
            GlStateManager.enableLighting();
            GlStateManager.disableBlend();
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            GlStateManager.popMatrix();
