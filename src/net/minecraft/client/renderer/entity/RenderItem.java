...
import net.theresa.neogenesis.modules.itemanimations.ModelFix;
...
public class RenderItem implements IResourceManagerReloadListener {
    ...
    protected void renderItemModelTransform(ItemStack stack, IBakedModel model,
        ...
        float x = ItemAnimations.Instance.getToggled() ? (float) (ModelFix.Instance.getScrollValue() / 200000.0f) : 0.0F;
        if (x != 0.0f && ItemRenderer.isRenderingArm && !ItemRenderer.isItem3D) {
            float factor = 0.1f;
            GlStateManager.translate(x, x, -x * factor);
            this.renderItem(stack, model);
            GlStateManager.translate(-2.0f * x, -2.0f * x, 2.0f * x * factor);
            this.renderItem(stack, model);
            GlStateManager.translate((2.0f + factor) * x, -factor * x, x * factor);
            this.renderItem(stack, model);
            GlStateManager.translate((-2.0f - 2.0f * factor) * x, (2.0f + 2.0f * factor) * x, -4.0f * x * factor);
            this.renderItem(stack, model);
            GlStateManager.translate((1.0f + factor) * x, (-1.0f - factor) * x, 2.0f * x * factor);
        }
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    private void registerItems() {
        ...
        this.itemModelMesher.register(Items.enchanted_book,
                stack -> new ModelResourceLocation("enchanted_book", "inventory"));
