...
public class EffectRenderer {
    ...
    private void tickParticle(final EntityFX particle) {
        try {
            ...
            crashreportcategory.addCrashSectionCallable("Particle Type", () -> i == 0 ? "MISC_TEXTURE"
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 2; ++j) {
                ...
                if (!this.fxLayers[i][j].isEmpty()) {
                    ...
                    for (EntityFX element : this.fxLayers[i][j]) {
                        final EntityFX entityfx = element;
    ...
    public void renderLitParticles(Entity entityIn, float partialTick) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (!list.isEmpty()) {
                ...
                for (EntityFX element : list) {
                    EntityFX entityfx = element;
            ...
            for (int i = 0; i < l; ++i) {
                for (int j = 0; j < l; ++j) {
                    for (int k = 0; k < l; ++k) {
                        ...
                        this.addEffect(new EntityDiggingFX(this.worldObj, d0, d1, d2, d0 - pos.getX() - 0.5D,
                                d1 - pos.getY() - 0.5D, d2 - pos.getZ() - 0.5D, state).setBlockPos(pos));
                    }
                }
            }
        }
    ...
    public void addBlockHitEffects(BlockPos pos, EnumFacing side) {
        ...
        if (block.getRenderType() != -1) {
            ...
            float f = 0.1F;
            double d0 = i
                    + this.rand.nextDouble() * (block.getBlockBoundsMaxX() - block.getBlockBoundsMinX() - f * 2.0F) + f
                    + block.getBlockBoundsMinX();
            double d1 = j
                    + this.rand.nextDouble() * (block.getBlockBoundsMaxY() - block.getBlockBoundsMinY() - f * 2.0F) + f
                    + block.getBlockBoundsMinY();
            double d2 = k
                    + this.rand.nextDouble() * (block.getBlockBoundsMaxZ() - block.getBlockBoundsMinZ() - f * 2.0F) + f
    ...
    public void addBlockHitEffects(BlockPos p_addBlockHitEffects_1_, MovingObjectPosition p_addBlockHitEffects_2_) {
        ...
        if ((iblockstate != null) && (iblockstate != null)) {
