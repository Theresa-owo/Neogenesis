...
public class EntityParticleEmitter extends EntityFX {
    ...
    public void onUpdate() {
        if (this.attachedEntity.getUniqueID().equals(Registry.Companion.getMc().thePlayer.getUniqueID())) {
            if (DisableSelfCritParticle.Instance.getToggled()) {
        ...
        int num = 16;
        if (this.particleTypes == EnumParticleTypes.CRIT_MAGIC || this.particleTypes == EnumParticleTypes.CRIT) {
            num = (int) (16 * (ParticleMultiplier.Instance.getToggled() ? ParticleMultiplierValue.Instance.getScrollValue() : 1.0));
        }
        ...
        for (int i = 0; i < num; ++i) {
            ...
            if (d0 * d0 + d1 * d1 + d2 * d2 <= 1.0D) {
                ...
                double d4 =
                        this.attachedEntity.getEntityBoundingBox().minY + (double) (this.attachedEntity.height / 2.0F) +
