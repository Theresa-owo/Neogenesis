...
public abstract class EntityLiving extends EntityLivingBase {
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound) {
        ...
        for (ItemStack element : this.equipment) {
            ...
            if (element != null) {
                element.writeToNBT(nbttagcompound);
            }
        ...
        for (float element : this.equipmentDropChances) {
            nbttaglist1.appendTag(new NBTTagFloat(element));
        }
    ...
    public final boolean interactFirst(EntityPlayer playerIn) {
        ...
        if (itemstack != null && itemstack.getItem() == Items.lead && this.allowLeashing()) {
            if (!(this instanceof EntityTameable) || !((EntityTameable) this).isTamed()
                    || ((EntityTameable) this).isOwner(playerIn)) {
    ...
    private boolean canSkipUpdate() {
        if (this.isChild() || this.hurtTime > 0 || this.ticksExisted < 20) {
            return false;
        }
        World world = this.getEntityWorld();

        if ((world == null) || (world.playerEntities.size() != 1)) {
            return false;
        } else {
